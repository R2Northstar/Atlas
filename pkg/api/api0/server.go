	...
	if !nsrv.VerificationDeadline.IsZero() {
		...
		if nsrv.AuthPort != 0 {
			if err := api0gameserver.Verify(ctx, s.AuthAddr()); err != nil {
				...
				respFail(w, r, http.StatusBadRequest, code.MessageObjf("failed to connect to auth port (addr %s): %v", nsrv.AuthAddr(), err))
				return
			}
		}
		...
		if err := h.probeUDP(ctx, s.Addr); err != nil {
			...
			h.m().server_upsert_verify_time_seconds.failure.UpdateDuration(verifyStart)
			respFail(w, r, http.StatusBadRequest, obj)
			return
		}
		...
		if !h.ServerList.VerifyServer(nsrv.ID) {
			h.m().server_upsert_requests_total.reject_verify_udptimeout(action).Inc()
			respFail(w, r, http.StatusBadRequest, ErrorCode_NO_GAMESERVER_RESPONSE.MessageObjf("verification timed out (addr %s)", nsrv.Addr))
			return
		}
